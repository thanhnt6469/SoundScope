...
async def process_audio(file: UploadFile = File(...)):
    try:
        ...
        if not os.path.exists(json_label_path):
            # If captioning failed, create empty result instead of failing completely
            print(f"Warning: captioning_deepfake.json not found for {file.filename}, creating empty result")
            save_dir = os.path.join(USER_FINAL_INFO_DIR, file.filename[:-4])
            os.makedirs(save_dir, exist_ok=True)
            empty_data = [{"audio_captioning": [], "deepfake_detection": []}]
